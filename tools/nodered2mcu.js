...
export default class extends TOOL {
	...
	transformFlows(flows) {
		...
		parts.push(`import structuredClone from "structuredClone"`);
		parts.push(``);
		...
		flows.forEach(config => {
			...
			flows.forEach(config => {
				...
				parts.push(`\tcreateNode("${type}", "${config.id}", ${JSON.stringify(config.name ?? "")}, flow);`) 
			});
		});
			...
			flows.forEach(config => {
				...
				try {
					...
					let errors = flows.filter(config => {
						if ((config.z !== z) || config.d || (config.type !== "catch"))
							return;
