...
class NRF52GATTFile extends GATTFile {
	constructor(dictionary) {
		super(dictionary);
		this.vendorSpecificUUIDs = [];
	}
	generate() {
		let tool = this.tool;
		let file = this.file;
		...
		if (0 == services.length) {
			...
			file.line("static const char_name_table char_names[0] = {};");
			file.line("static const vendor_specific_uuid_t vendor_uuids[0] = {};");
			if (this.server) {
				file.line("");
		...
		services.forEach((service, index) => {
			...
			file.line("\t\t},");
			this.addUniqueVendorSpecificUUID(service.uuid, index, "service");
			++attributeIndex;
			...
			for (let key in characteristics) {
				...
				char_name.type = characteristic.type ? characteristic.type: "";
				char_names.push(char_name);
				file.write(`\t\t\t{${uuid_len}, (uint8_t*)&char_uuid${characteristicIndex}, ${permissions}, ${maxBytes}, `);
				if ("value" in characteristic)
					file.write(`${characteristic._length}, (uint8_t*)&char_value${characteristicIndex}}`);
				else
					file.write("0, NULL}");
				file.line("");
				file.line("\t\t},");
				this.addUniqueVendorSpecificUUID(characteristic.uuid, characteristicIndex, "char");
				++attributeIndex;
				...
				if (characteristic._descriptors) {
					for (let key2 in characteristic.descriptors) {
						...
						file.line("");
						file.line("\t\t},");
						this.addUniqueVendorSpecificUUID(descriptor.uuid, descriptorIndex, "desc");
						++descriptorIndex;
						++attributeIndex;
					}
				}
		...
		let vendorSpecificUUIDs = this.vendorSpecificUUIDs;
		file.line(`#define vendor_specific_uuid_count ${vendorSpecificUUIDs.length}`);
		file.line(`static const vendor_specific_uuid_t vendor_uuids[${vendorSpecificUUIDs.length}] = {`);
		vendorSpecificUUIDs.forEach(entry => {
			file.line(`\t(uint8_t*)&${entry.prefix}_uuid${entry.index},`);
		});
		file.line("};");
		file.line("");
		file.line(`#define char_name_count ${char_names.length}`);
	...
	parseProperties(properties) {
		...
		const GATT_CHAR_PROP_BIT_WRITE_NR = (1 << 2);
		const GATT_CHAR_PROP_BIT_WRITE = (1 << 3);
		const GATT_CHAR_PROP_BIT_NOTIFY = (1 << 4);
		const GATT_CHAR_PROP_BIT_INDICATE = (1 << 5);
		const GATT_CHAR_PROP_BIT_AUTH = (1 << 6);
		const GATT_CHAR_PROP_BIT_EXT_PROP = (1 << 7);
		let props = 0;
		properties.forEach(p => {
			switch(p.trim()) {
				case "read":
					props |= GATT_CHAR_PROP_BIT_READ;
					break;
				case "write":
					props |= GATT_CHAR_PROP_BIT_WRITE;
					break;
				case "writeNoResponse":
					props |= GATT_CHAR_PROP_BIT_WRITE_NR;
					break;
				case "notify":
					props |= GATT_CHAR_PROP_BIT_NOTIFY;
					break;
				case "indicate":
					props |= GATT_CHAR_PROP_BIT_INDICATE;
					break;
				case "extended":
					props |= GATT_CHAR_PROP_BIT_EXT_PROP;
					break;
				default:
					throw new Error("unknown property");
			}
	...
	parsePermissions(permissions) {
		...
		permissions.forEach(p => {
			switch(p.trim()) {
				case "read":
					++readPerms;
					perms.push("GATT_PERM_READ");
					break;
				case "readEncrypted":
					++readPerms;
					perms.push("GATT_PERM_READ_ENCRYPTED");
					break;
				case "write":
					++writePerms;
					perms.push("GATT_PERM_WRITE");
					break;
				case "writeEncrypted":
					++writePerms;
					perms.push("GATT_PERM_WRITE_ENCRYPTED");
					break;
				default:
					throw new Error("unsupported permission");
			}
	...
	addUniqueVendorSpecificUUID(uuid, index, prefix) {
		if (36 !== uuid.length) return;
		let vendorSpecificUUIDs = this.vendorSpecificUUIDs;
		let buffer = new Uint8Array(uuid128toBuffer(uuid));
		let match;
		if ("service" == prefix) {
			match = vendorSpecificUUIDs.find(entry => { entry.buffer[12] == buffer[12] && entry.buffer[13] == buffer[13] });
		}
		else {
			match = vendorSpecificUUIDs.find(entry => {
				let equal = true;
				for (let i = 0; equal && (i < 16); ++i) {
					if (12 == i || 13 == i)
						continue;
					equal = entry.buffer[i] == buffer[i];
				}
				return equal;
			});
		}
		if (!match)
			vendorSpecificUUIDs.push({ index, prefix, buffer });
	}
};
...
export default class extends TOOL {
		...
		for (var argi = 1; argi < argc; argi++) {
			...
			switch (option) {
				case "-o":
					...
					else if (this.outputPath.includes('tmp/nrf52') || this.outputPath.includes('tmp\\nrf52'))
						this.platform = "nrf52";
					else
	...
	run() {
		...
		else if ("nrf52" == this.platform)
			gatt = new NRF52GATTFile(dictionary);
		gatt.generate();
		file.close();
	}
}
