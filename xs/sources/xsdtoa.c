...
txNumber fxStringToBigInt(void* the, txString theString, txFlag whole)
{
	bn_t digit = { .sign=0, .size=1, .data={ 0 } };
	bn_t nan = { .sign=0, .size=0, .data={ 0 } };
	bn_t ten = { .sign=0, .size=1, .data={ 10 } };
	bn_t zero = { .sign=0, .size=1, .data={ 0 } };
	bn_t *result = bn_dup(the, &zero);
	txString p = fxSkipSpaces(theString), q;
	char c = *p;
	if (c) {
		txU4 d = *(p + 1);
		if (whole && (c == '0') && ((d == 'B') || (d == 'b') || (d == 'O') || (d == 'o') || (d == 'X') || (d == 'x'))) {
			...
			if ((d == 'B') || (d == 'b')) {
				while ((c = *p)) {
					if (('0' <= c) && (c <= '1')) {
						digit.data[0] = c - '0';
						result = bn_add(the, C_NULL, bn_lsl(the, C_NULL, result, 1), &digit);
					}
					else
						break;
					p++;
				...
				while ((c = *p)) {
					if (('0' <= c) && (c <= '7')) {
						digit.data[0] = c - '0';
						result = bn_add(the, C_NULL, bn_lsl(the, C_NULL, result, 3), &digit);
					}
					else
						break;
					p++;
				...
				while ((c = *p)) {
					if (('0' <= c) && (c <= '9')) {
						digit.data[0] = c - '0';
						result = bn_add(the, C_NULL, bn_lsl(the, C_NULL, result, 4), &digit);
					}
					else if (('a' <= c) && (c <= 'f')) {
						digit.data[0] = 10 + c - 'a';
						result = bn_add(the, C_NULL, bn_lsl(the, C_NULL, result, 4), &digit);
			...
			while ((c = *p)) {
				if (('0' <= c) && (c <= '9')) {
					digit.data[0] = c - '0';
					result = bn_add(the, C_NULL, bn_mul(the, C_NULL, result, &ten), &digit);
				}
				else
					break;
			}
