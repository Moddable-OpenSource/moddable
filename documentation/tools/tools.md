...
> **Note**: Earlier versions of the Moddable SDK use a different simulator, named "Screen Test". The Screen Test simulator is no longer being maintained and will be removed. To run "Screen Test" use `-p screentest` on the `mcconfig` command line.