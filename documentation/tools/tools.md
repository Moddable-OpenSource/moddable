...
This documents presents the tools provided by Moddable to build, debug and run JavaScript apps on micro-controllers or in the Moddable simulator, **Screen Test**.
...
The first app will take some time to build since all the ESP and XS libraries need to be compiled too. Such libraries are shared, so the next apps will build faster.
...
- `-p platform`: to select the platform: `esp` or `mac`. Defaults to `mac`
...
By default **xsc** parse the JS file as an ECMAScript module. Optionnaly, for the sake of compatibility and conformance, **xsc** can parse the JS file as an ECMAScript program or a CommonJS module. Moddable apps only uses ECMAScript modules.
...
### Arguments
	...
	xsc file [-c] [-d] [-d] [-m] [-o directory] [-p] [-r name] [-t directory]
