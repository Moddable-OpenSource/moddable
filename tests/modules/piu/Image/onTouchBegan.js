...
const sampleContent = new Image(null, {
	top: 0, left: 0, height: 50, width: 50, path: "screen2.cs",
...
.then(async () => {
	...
	for (let coordinate of touching) {
		...
		await screen.doTouchBegan(0, coordinate.x, coordinate.y, Time.ticks);
		await screen.doTouchEnded(0, coordinate.x, coordinate.y, Time.ticks);
		assert.sameValue("called", expected, "onTouchBegan not called");
	}
}).then($DONE, $DONE);
