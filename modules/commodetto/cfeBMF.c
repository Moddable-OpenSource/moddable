...
{
	const unsigned char *bytes, *start, *end;
	uint32_t size;
	uint8_t version;
	...
	bmf->isContinuous = (size && (0x40 & c_read8(19 + bmf->charTable))) ? 1 : 0;
	bmf->charCount = size / 20;

#if MODDEF_CFE_KERN
	// block 5 - kerning
	bytes += 4 + size;
	if (((end - bytes) >= 1) && (5 == c_read8(bytes))) {
		bytes += 1;
		if ((end - bytes) < 4)
			return;
		bmf->kernCount = c_read32(bytes) / 10;
		bmf->kernTriples = bytes + 4;
		bmf->kernSorted = (size && (0x80 & c_read8(19 + bmf->charTable))) ? 1 : 0;
	}
#endif
