...
/*
	Changes for Moddable SDK:

		- add c_ prefix to stdlib calls (strlen, etc)
*/
...
static inline void lfs_cache_zero(lfs_t *lfs, lfs_cache_t *pcache) {
    ...
    c_memset(pcache->buffer, 0xff, lfs->cfg->cache_size);
    ...
    while (size > 0) {
        ...
        if (pcache && block == pcache->block &&
            ...
            if (off >= pcache->off) {
                ...
                c_memcpy(data, &pcache->buffer[off-pcache->off], diff);
        ...
        if (block == rcache->block &&
            ...
            if (off >= rcache->off) {
                ...
                c_memcpy(data, &rcache->buffer[off-rcache->off], diff);
...
static int lfs_bd_prog(lfs_t *lfs,
    ...
    while (size > 0) {
        if (block == pcache->block &&
            ...
            c_memcpy(&pcache->buffer[off-pcache->off], data, diff);
...
static int lfs_alloc(lfs_t *lfs, lfs_block_t *block) {
    while (true) {
        ...
        c_memset(lfs->free.buffer, 0, lfs->cfg->lookahead_size);
...
static lfs_stag_t lfs_dir_getslice(lfs_t *lfs, const lfs_mdir_t *dir,
    ...
    while (off >= sizeof(lfs_tag_t) + lfs_tag_dsize(ntag)) {
        ...
        if ((gmask & tag) == (gmask & (gtag - gdiff))) {
            ...
            c_memset((uint8_t*)gbuffer + diff, 0, gsize - diff);
...
nextname:
        // skip slashes
        name += c_strspn(name, "/");
        lfs_size_t namelen = c_strcspn(name, "/");

        // skip '.' and root '..'
        if ((namelen == 1 && c_memcmp(name, ".", 1) == 0) ||
            (namelen == 2 && c_memcmp(name, "..", 2) == 0)) {
            name += namelen;
            goto nextname;
        }
        ...
        int depth = 1;
        while (true) {
            suffix += c_strspn(suffix, "/");
            sufflen = c_strcspn(suffix, "/");
            if (sufflen == 0) {
                break;
            }

            if (sufflen == 2 && c_memcmp(suffix, "..", 2) == 0) {
                depth -= 1;
                if (depth == 0) {
                    name = suffix + sufflen;
                    goto nextname;
...
static int lfs_file_relocate(lfs_t *lfs, lfs_file_t *file) {
    while (true) {
        ...
        c_memcpy(file->cache.buffer, lfs->pcache.buffer, lfs->cfg->cache_size);
...
static int lfs_rawrename(lfs_t *lfs, const char *oldpath, const char *newpath) {
    ...
    if (prevtag == LFS_ERR_NOENT) {
        // check that name fits
        lfs_size_t nlen = c_strlen(newpath);
        if (nlen > lfs->name_max) {
            return LFS_ERR_NAMETOOLONG;
        }
    ...
    err = lfs_dir_commit(lfs, &newcwd, LFS_MKATTRS(
            ...
            {LFS_MKTAG(lfs_tag_type3(oldtag), newid, c_strlen(newpath)), newpath},
    ...
    while (!lfs_pair_isnull(cwd.d.tail)) {
        ...
        while (true) {
            ...
            if (!(0x80 & entry.d.type) &&
                 c_memcmp(&entry.d.u, e, sizeof(entry.d.u)) == 0) {
                return true;
            }
        }
    }
...
static int lfs1_mount(lfs_t *lfs, struct lfs1 *lfs1,
    ...
    int err = 0;
    {
        ...
        if (err || c_memcmp(superblock.d.magic, "littlefs", 8) != 0) {
