...
export default class Client {
  constructor(dictionary) {
    if (!dictionary.id) throw new Error('parameter id is required');

    if (!dictionary.host) throw new Error('parameter host is required');

    this.connect = { id: dictionary.id };
    if (dictionary.user) this.connect.user = dictionary.user;
    if (dictionary.password) this.connect.password = dictionary.password;
    ...
    if (dictionary.timeout) this.timeout = dictionary.timeout;
    ...
    if (this.path) {
      ...
      if (dictionary.Socket)
        this.ws = new dictionary.Socket({
          host: dictionary.host,
          port,
          secure: dictionary.secure
        });
      else this.ws = new Socket({ host: dictionary.host, port });
      this.ws.callback = socket_callback.bind(this);
    }
  }
  publish(topic, data) {
    if (this.state < 2) throw new Error('cannot publish to closed connection');

    ++this.packet;

    topic = makeStringBuffer(topic);
    if (!(data instanceof ArrayBuffer)) data = ArrayBuffer.fromString(data);
  ...
  received(buffer) {
    ...
    while (position < length) {
      switch (parse.state) {
        ...
        case 4:
          if (buffer[position] & 0x80) throw new Error('bad data');
        ...
        case 0x31:
          ...
          if (0 === parse.topic) throw new Error('empty topic - ?!?');
        ...
        case 0x37:
          if (parse.length) {
            ...
            if (use > length - position) use = length - position;
            parse.payload.set(
              new Uint8Array(buffer.buffer, position, use),
              parse.payload.position
            );
        ...
        // CONNACK
        case 0x20:
          if (2 !== parse.length) throw new Error('unexpected length');
        ...
        case 0x92: // ignore remaining bytes
          position += 1;
          parse.length -= 1;
          if (0 === parse.length) parse = this.parse = { state: 0 }; // not dispatched
          break;
    ...
    if (this.timer) this.last = Date.now();
  }
  connected() {
    ...
    const header = Uint8Array.of(
      ...
      'Q'.charCodeAt(),
      'T'.charCodeAt(),
      'T'.charCodeAt(), // protocol name MQTT
      0x04, // protocol level 4
      0x02, // flags : CleanSession
      (timeout >> 8) & 0xff,
      timeout & 0xff // keepalive in seconds
    );
    ...
    msg[position + 7] |=
      (user.length ? 0x80 : 0) | (password.length ? 0x40 : 0);
    position += header.length;

    msg.set(id, position);
    position += id.length;
    msg.set(user, position);
    position += user.length;
    msg.set(password, position);
  ...
  dispatch(msg) {
    if (1 === this.state) {
      if (msg.code != CONNACK) {
        trace(
          `WARNING: received message type '${flag}' when expecting CONNACK\n`
        );
        return;
      }
  ...
  keepalive() {
    ...
    let now = Date.now();
    if (this.last + this.timeout > now) return; // received data within the timeout interval

    if (this.last + (this.timeout + (this.timeout >> 1)) > now)
      this.ws.write(Uint8Array.of(0xc0, 0x00).buffer);
    // ping
    else {
      // timed out after 1.5x waiting
      this.onClose();
      this.close();
    }
  }
  ...
  switch (state) {
    ...
    default:
      if (state < 0) {
        ...
        delete this.ws;
        this.close();
      } else trace(`ERROR: unhandled socket state ${state}\n`);
      break;
  }
}

function makeStringBuffer(string) {
  if (undefined === string) return new Uint8Array(0);

  if (!(string instanceof ArrayBuffer)) string = ArrayBuffer.fromString(string);
...
function getRemainingLength(length) {
  ...
  else if (length < 16384) return 2;
  else if (length < 2097152) return 3;
  return 4;
}

function writeRemainingLength(length, buffer, position) {
  if (length < 128) buffer[position++] = length;
