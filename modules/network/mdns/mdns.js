...
class MDNS extends Socket {
  ...
  close() {
    while (this.services.length) this.remove(this.services[0]);

    while (this.monitors.length)
      this.remove(
        this.monitors[0].service.substring(
          0,
          this.monitors[0].service.length - 6
        )
      );
  ...
  add(service) {
    if (!this.hostName) throw new Error('no hostName');

    if (this.probing) throw new Error('not ready');

    if (!service.txt) service.txt = {};
    this.services.push(service);

    this.write(
      MDNS.IP,
      MDNS.PORT,
      this.reply(null, 0x1f | 0x8000, service, true)
    ); // remove before adding to clear stale state in clients
    service.timer = Timer.repeat(timer => {
      ...
      } else Timer.schedule(timer, timer.interval, timer.interval);
    }, 500);
  ...
  update(service) {
    ...
    if (!service.updater) {
      service.updater = Timer.set(
        updater => {
          this.write(
            MDNS.IP,
            MDNS.PORT,
            this.reply(null, 0x04 | 0x8000, service)
          );
          ...
          } else Timer.schedule(updater, updater.interval, 250);
        },
        0,
        250
      );
    } else Timer.schedule(service.updater, 0, 250);
  ...
  remove(service, callback) {
    if ('string' === typeof service) {
      ...
      if (service >= 0) {
        callback = this.monitors[service].callbacks.findIndex(
          item => item === callback
        );
  ...
  monitorTask() {
    ...
    switch (this.state) {
      ...
      case 3:
        ...
        query.add(
          DNS.SECTION.QUESTION,
          this.service,
          DNS.RR.PTR,
          DNS.CLASS.IN | (0 == this.state ? MDNS.UNICAST : 0)
        );
  ...
  scanPacket(packet, address) {
    ...
    for (i = 0, length = answers + packet.additionals; i < length; i++) {
      const record =
        i < answers ? packet.answer(i) : packet.additional(i - answers);
        ...
        case DNS.RR.A:
          ...
          this.monitors.forEach(monitor => {
            monitor.forEach(instance => {
              if (instance.target === name) instance.address = record.rdata;
            });
          });
          break;
        case DNS.RR.PTR:
          ...
          if (DNS.RR.TXT === record.qtype) {
            ...
            if (!instance.txt || instance.txt.length !== txt.length) {
              ...
              for (j = 0; j < txt.length; ++j) {
                if (txt[j] === instance.txt[j]) continue;
                instance.txt = txt;
                instance.changed = changed = true;
                break;
              }
        ...
        if (
          ...
          for (let k = 0; k < callbacks.length; k++) {
            ...
            if (monitor.callbacks.indexOf(callback) < 0)
              // expensive check to see if callback was closed
              continue;
          ...
          if (instance.target)
            query.add(
              ...
              instance.target,
              DNS.RR.A,
              DNS.CLASS.IN | MDNS.UNICAST
            );
          query.add(
            DNS.SECTION.QUESTION,
            instance.name + '.' + monitor.service,
            DNS.RR.SRV,
            DNS.CLASS.IN | MDNS.UNICAST
          );
          query.add(
  ...
  callback(message, value, address, port) {
    ...
    if (packet.answers || packet.additionals) this.scanPacket(packet, address);
    ...
    if (this.probing) {
      ...
      for (let i = 0; i < packet.questions; i++) {
        ...
        if (DNS.RR.ANY !== question.qtype) continue;
        if (
          this.hostName !== question.qname[0] ||
          2 !== question.qname.length ||
          LOCAL !== question.qname[1]
        )
          continue;
        ...
        for (let j = 0; j < packet.authorities; j++) {
          ...
          if (DNS.RR.A !== authority.qtype) continue;
          if (
            this.hostName !== authority.qname[0] ||
            2 !== authority.qname.length ||
            LOCAL !== authority.qname[1]
          )
            continue;
    ...
    if (packet.flags & 0x8000) return;

    for (
      let h = 0, length = this.services.length ? this.services.length : 1;
      h < length;
      h++
    ) {
      // always at least one pass, to pick up service-indepdent qtypes
      ...
      for (let i = 0; i < packet.questions; i++) {
        ...
        switch (question.qtype) {
          ...
          case DNS.RR.PTR:
            if (!service);
            else if (3 === name.length && LOCAL === name[2]) {
              // service query
              if (
                '_' + service.name === name[0] &&
                '_' + service.protocol === name[1]
              ) {
                let respond = true;

                for (let j = 0; j < packet.answers && respond; j++) {
                  let answer = packet.answer(j);
                  if (DNS.RR.PTR !== answer.qtype) continue;
                  if (
                    3 !== answer.qname.length ||
                    '_' + service.name !== answer.qname[0] ||
                    '_' + service.protocol !== answer.qname[1] ||
                    LOCAL !== answer.qname[2]
                  )
                    continue;
                  if (
                    4 !== answer.rdata.length ||
                    this.hostName !== answer.rdata[0] ||
                    '_' + service.name !== answer.rdata[1] ||
                    '_' + service.protocol !== answer.rdata[2] ||
                    LOCAL !== answer.rdata[3]
                  )
                    continue;
                  respond = false;
                }
                if (respond) mask[question.qclass >> 15] |= 0x1f;
              }
            } else if (
              4 === name.length &&
              '_services' === name[0] &&
              ...
              for (let j = 0; j < packet.answers && respond; j++) {
                ...
                if (
                  3 === answer.rdata.length &&
                  '_' + service.name === answer.rdata[0] &&
                  '_' + service.protocol === answer.rdata[1] &&
                  LOCAL === answer.rdata[2]
                )
                  respond = false;
              }
              if (respond) mask[question.qclass >> 15] |= 0x10;
              break;
            }
            break;
          ...
          case DNS.RR.SRV:
            if (!service);
              ...
              LOCAL === name[3] &&
              this.hostName === name[0] &&
              '_' + service.name === name[1] &&
              '_' + service.protocol === name[2]
            )
    ...
    if (response[1]) this.write(address, port, response[1].build(packet.id));
  }
  ...
  reply(response, mask, service, bye = false) {
    ...
    if (0x20 & mask)
      // NSEC indicating A only
      response.add(
        DNS.SECTION.ANSWER,
        this.hostName + '.' + LOCAL,
        DNS.RR.NSEC,
        DNS.CLASS.IN | MDNS.FLUSH,
        TTL & bye,
        {
          next: this.hostName + '.' + LOCAL,
          bitmaps: Uint8Array.of(0, 4, 0x40, 0, 0, 0)
        }
      );

    if (0x10 & mask)
      // PTR for service discovery
      response.add(
        DNS.SECTION.ANSWER,
        ...
        DNS.CLASS.IN,
        TTL & bye,
        '_' + service.name + '._' + service.protocol + '.' + LOCAL
      );
    ...
    if (0x04 & mask)
      ...
      response.add(
        ...
        this.hostName +
          ...
          LOCAL,
        DNS.RR.TXT,
        DNS.CLASS.IN | (0x8000 & mask ? MDNS.FLUSH : 0),
        TTL & bye,
        service.txt
      );
    ...
    Timer.repeat(
      id => {
        ...
        reply.add(
          ...
          DNS.RR.ANY,
          DNS.CLASS.IN | MDNS.UNICAST
        ); // question for DNS.RR.A, unicast response requested
        reply.add(
          DNS.SECTION.ANSWER,
          this.hostName + '.' + LOCAL,
          DNS.RR.A,
          ...
          120,
          Net.get('IP')
        ); // authoritative answer for DNS.RR.A
