...
class Connection extends Socket {
    ...
    switch (message) {
      case 2: // data ready
        while (count--) {
          ...
          if (this.command) {
            if (true === this.command) {
              if (244 === byte) {
                ...
                if (this.suspended) {
                  if (true !== this.suspended) this.suspended();
          ...
          if (8 === byte || 127 === byte) {
            //@@ is this ever used @@
            if (this.incoming.length) this.incoming.length -= 1;
            continue;
          }
          if (10 === byte) {
            CLI.execute.call(this, this.incoming);
            this.incoming = '';
            if (!this.suspended) CLI.execute.call(this, 'prompt');
            continue;
          }
  ...
  close() {
    ...
    if (index >= 0) this.listener.connections.splice(index, 1);
  }
  // placeholders
  suspend(cancel) {
