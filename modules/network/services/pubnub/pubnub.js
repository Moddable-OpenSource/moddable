...
export class PubNub {
  constructor(params) {
    this.config = {
      authority: 'pubsub.pubnub.com',
      pnsdk: 'Moddable-XS6/1',
      uuid: UUID()
    };
    ...
    REST_API.subscribe(
      ...
      (error, data) => {
        ...
        if (error) trace('Subscribe Error: ' + error + '\n');
          ...
          trace('Subscribe Error: unexpected data type: ' + typeof data + '\n');
        if (callback) {
          if (scope) callback.bind(scope);
          callback(error, data);
        }
...
export class REST_API {
  static fetchHistory(config, params, callback, scope) {
    ...
    params.location = `/v2/history/sub-key/${
      params.sub_key
    }/channel/${channel}`;
  ...
  static publish(config, params, callback, scope) {
    ...
    let channel = encodeURIComponent(params.channel);
    params.location = `/publish/${params.pub_key}/${
      params.sub_key
    }/0/${channel}/0`;
    return makeRequest(params, callback, scope);
  }
