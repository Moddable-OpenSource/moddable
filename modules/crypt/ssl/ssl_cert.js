...
class CertificateManager {
  constructor(options) {
    ...
    if (options.certificate) this.register(options.certificate);
  }
  getCerts() {
  ...
  getKey(cert) {
    if (!cert) {
      ...
      return PKCS8.decrypt(
        getResource('srvkey.pk8'),
        Configuration.pk8Password
      );
    }
  ...
  getIndex(fname, target) {
    if (undefined === target) return -1;

    let f = new Resource(fname);
    for (let i = 0; (i + 1) * 20 <= f.byteLength; i++) {
      if (Bin.comp(f.slice(i * 20, (i + 1) * 20), target) == 0) return i;
    }
    return -1;
  }
  findCert(fname, target) {
    ...
    let validity = X509.decodeTBS(X509.decode(new Uint8Array(data)).tbs)
      .validity;
    let now = Date.now();
  ...
  verify(certs) {
    ...
    for (let i = 0; i < length; i++) {
      ...
      if (!this._verify(X509.decodeSPKI(certs[i + 1]), x509)) return false;
      ...
      aki = undefined;
      if (spki && this._verify(spki, X509.decode(certs[i + 1]))) return true;
      // else fall thru
    }
    ...
    if (spki && this._verify(spki, x509)) return true;
    // else fall thru

    spki = this.findCert('ca.subject', new Crypt.Digest('SHA1')).process(
      X509.decodeTBS(x509.tbs).issuer
    );
  ...
  register(cert) {
    let validity = X509.decodeTBS(X509.decode(new Uint8Array(cert)).tbs)
        .validity,
      now = Date.now();
    if (!(validity.from < now && now < validity.to))
      throw new Error('date validation failed');
