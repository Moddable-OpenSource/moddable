...
class SSLSession {
  constructor(options = {}) {
    if (options.protocolVersion) {
      if (
        options.protocolVersion < minProtocolVersion ||
        options.protocolVersion > maxProtocolVersion
      )
    ...
    if ('applicationLayerProtocolNegotiation' in options) {
      options.tls_application_layer_protocol_negotiation =
        options.applicationLayerProtocolNegotiation;
    ...
    this.minProtocolVersion =
      this.options.protocolVersion || minProtocolVersion; // only for the server side
    this.maxProtocolVersion = maxProtocolVersion; // ditto
    if (this.options.trace) this.traceLevel = 0;
    this.applicationData = undefined;
    this.cacheManager =
      (this.options.cache === undefined || this.options.cache) && cacheManager;
  ...
  handshake(s, n) {
    ...
    switch (this.handshakeProcess) {
      case handshakeProtocol.helloRequest.msgType:
        {
          // C
          let cache =
            this.cacheManager &&
            this.cacheManager.getByHost(this.options.tls_server_name);
          if (cache) {
        ...
        break;
      case handshakeProtocol.clientHello.msgType:
        {
          // S
          let masterSecret =
            this.clientSessionID &&
            this.cacheManager &&
            this.cacheManager.getByID(this.clientSessionID);
          if (masterSecret) {
            // resumed handshake
            ...
            // assign a new one
            this.serverSessionID = ArrayBuffer.fromBigInt(
              BigInt(new Date().valueOf().toString())
            );
            this.doProtocol(s, handshakeProtocol.serverHello);
            ...
            if (this.options.clientAuth)
              ...
              this.doProtocol(
                s,
                handshakeProtocol.certificateRequest,
                options.clientAuth.cipherSuites,
                options.clientAuth.subjectDN
              );
            // S -> C: ServerHelloDone
      ...
      case handshakeProtocol.serverHelloDone.msgType: // C
        ...
        if (this.myCert)
          // client cert request && and the certs is not empty
          this.doProtocol(s, handshakeProtocol.certificateVerify, this.myCert);
        this.doProtocol(s, changeCipherSpec);
        this.doProtocol(s, handshakeProtocol.finished);
        break;
      case handshakeProtocol.finished.msgType:
        {
          // C, S
          let resumed =
            this.clientSessionID &&
            Bin.comp(this.clientSessionID, this.serverSessionID) == 0;
    ...
    if (state == 0) this.handshakeProcess = -1;
    if (state == 2) {
      this.stopTrace();
      return true;
    } else return false;
  ...
  read(s, n) {
    ...
    if (
      !applicationData ||
      0 == applicationData.byteLength ||
      applicationData.position >=
        applicationData.byteLength + applicationData.byteOffset
    ) {
      // read at least one packet and just keep it
      let buf = this.readPacket(s, s.read()); // bytesAvailable
      if (!buf) return applicationData; // return an empty buffer
      this.startTrace('unpacketize');
      recordProtocol.unpacketize(this, buf);
      if (this.alert) return null; // probably the session has been closed by the peer
    }
    if (n === undefined) return this.applicationData; // return the whole data whether it is empty or not -- DO NOT modify the content
  ...
  write(s, data) {
    if (data.byteLength > maxFragmentSize) return -1; // too large
    this.startTrace('packetize');
    s.write(
      recordProtocol.packetize(this, recordProtocol.application_data, data)
    );
    return data.length;
  }
  close(s) {
  ...
  readPacket(s, available) {
    ...
    if (packetBuffer.length < 5) {
      ...
      this.packetBuffer = new Uint8Array(
        ((packetBuffer[3] << 8) | packetBuffer[4]) + 5
      ); /* packet length including 5 byte header */
      this.packetBuffer.set(packetBuffer);
      packetBuffer = this.packetBuffer;
    ...
    if (need) {
      ...
      if (packetBuffer.offset < packetBuffer.length) return;
    }
    this.packetBuffer = new Uint8Array();
  ...
  get bytesAvailable() {
    return this.applicationData
      ? this.applicationData.end - this.applicationData.position
      : 0;
  }
  putData(data) {
  ...
  startTrace(direction) {
    if (undefined === this.traceLevel) return;
  ...
  stopTrace() {
    if (this.alert) {
      trace(
        'ALERT! (' + this.alert.level + ', ' + htis.alert.description + '\n'
      );
      return;
    }
    if (undefined === this.traceLevel) return;
