...
txBigInt *fxBigInt_fromChunk(txMachine *the, void *data, txU4 size, txBoolean signess, txBoolean lsbFirst)
{
	int neg = 0;
	txBigInt *bn;
	unsigned char *os;

	bn = fxBigInt_alloc(the, howmany(size, sizeof(txU4)));
	os = data;
	if (lsbFirst) {
		txU4 i, j;
		for (i = 0, j = 0; i < size; i += 4, j++) {
#define str_data(n)	(n >= size ? 0 : (txU4)os[n])
...
#define MSB	(1 << (sizeof(txU4)*8 - 1))
		...
		for (i = (int)size, j = 0; i > 0; i -= 4, j++) {
#undef str_data
#define str_data(n)	((n) < 0 ? 0: (txU4)os[(n)])
...
txSlot *fxBigInt_toChunk(txMachine *the, txBigInt *a, txU4 minBytes, txBoolean signess, txBoolean lsbFirst)
{
	uint32_t nbytes, chunkSize;
	int32_t n;
	unsigned char *os;

	nbytes = howmany(fxBigInt_bitsize(a), 8);
	if (nbytes == 0)
		nbytes = 1;
	chunkSize = MAX(nbytes, minBytes);
	if (signess && !lsbFirst)
		chunkSize++;
	fxArrayBuffer(the, &the->scratch, NULL, chunkSize);
	os = fxToArrayBuffer(the, &the->scratch);
	/* all clear in case i2os won't fill out "nbytes" -- this would happen in libtom when the number is zero */
	c_memset(os, 0, chunkSize);
	if (lsbFirst) {
		int n;
		txU4 x;
		int i;
...
int fxBigInt_sizeof(txMachine *the, txBigInt *a)
{
	int n = fxBigInt_bitsize(a);
	return howmany(n, 8);
}
