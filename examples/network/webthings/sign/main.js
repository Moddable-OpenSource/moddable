...
class AppBehavior extends Behavior {
  ...
  onDisplaying(application) {
    ...
    if (config.ssid == 'YOUR_WIFI_SSID_HERE') {
      ...
      trace(
        'Error: no Wi-Fi credentials. Must include Wi-Fi credentials in manifest.json\n'
      );
      return;
    } else {
...
const SignApp = Application.template($ => ({
  ...
  top: 0,
  bottom: 0,
  Skin: ASSETS.MozillaBlueSkin,
  contents: [Text($, { left: 15, right: 15, Style: ASSETS.OpenSans28 })],
