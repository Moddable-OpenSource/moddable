...
const touch = new XPT2046;
touch.points = [{}];

let touches = [];
let state = 0;
const locations = [{x: 10, y: 10}, {x: 190, y: 10}, {x: 10, y: 270}, {x: 190, y: 270}];
let grays = 255;
...
Timer.repeat(id => {
	...
	let flipX, flipY;
	if ((touches[0].x > touches[1].x) && (touches[2].x > touches[3].x))
		 flipX = true;
	else if ((touches[0].x < touches[1].x) && (touches[2].x < touches[3].x))
		flipX = false;

	 if ((touches[0].y > touches[3].y) && (touches[1].y > touches[3].y))
		 flipY = true;
	else if ((touches[0].y < touches[3].y) && (touches[1].y < touches[3].y))
		 flipY = false;
	...
	let left = (touches[0].x + touches[2].x) / 2;
	let right = (touches[1].x + touches[3].x) / 2;
