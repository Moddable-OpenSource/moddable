...
class NoSleepBehavior extends View.Behavior {
...
const NoSleepContainer = Container.template($ => ({
	left:0, right:0, top:0, bottom:0, skin:assets.skins.screen, style:assets.styles.screen, Behavior:NoSleepBehavior,
	contents: [
		Container($, {
			...
			contents: [
				...
				Label($, {left:0, width:128, top:30, height:26, string:"sleeps" }),
			], 
		}),
		Container($, {
			left:0, width:128, top:64, height:64, skin:assets.skins.focus, state:1, visible:true,
			contents: [
				Label($, {left:0, width:128, top:4, height:26, state:1, string:"only in a" }),
				Label($, {left:0, width:128, top:30, height:26, state:1, string:"release build" }),
			], 
		}),
	]
}));

class NoSleepTimeline extends Timeline {
...
export default class extends View {
	static get Behavior() { return NoSleepBehavior }
	
	constructor(data) {
		super(data);
		this.wakenWith = null;
	}
	get Template() { return NoSleepContainer }
	get Timeline() { return NoSleepTimeline }
};
